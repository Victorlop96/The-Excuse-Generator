...
if (!fs.existsSync(path.resolve(__dirname, ".git"))) {
  ...
  Console.help(
    "Follow this steps to create a new repository for your project: http://kbroman.org/github_tutorial/pages/init.html"
  );
  return;
}
...
Console.info(
  "The remote was found successfully, starting the deploy from here: " + origin
);
...
compiler.run((err, stats) => {
  ...
  ghpages.publish("public", function(err) {
    ...
    Console.success(
      `Your website has been deployed successfully here: https://${repository["owner"]}.github.io/${repository["name"]}`
    );
  });
});
